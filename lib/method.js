...
function method(endpointSpec, methodParams, cb) {
  ...
  if (endpointSpec.auth) {
    return authorize(methodParams.authEmail, methodParams.authPassword, methodParams.token || methodParams.access_token, {}, function _authCb(authErr, authToken) {
      if (authErr) return cb(authErr);

      if (!methodParams.access_token)  {
        methodParams.access_token = authToken;
      }
